...
The queue must have been created through the specified device, but no command list is needed for this stage.
...
Event data needs to be collected and sent to the profiler by calling the \texttt{TracyD3D12Collect(ctx)} macro periodically, ideally after waiting on a fence or after a present (swap buffer) call. During collection, refrain from recording instrumented command lists in other threads, and from submitting instrumented command lists to the command queue.

Tracy is \textbf{not aware} of the order in which instrumeted command lists are submitted to the command queue, \textbf{if at all}. As a consequence, if an instrumented command list is never submitted for execution to the command queue, Tracy will be unable to properly collect and reclaim timestamp queries, \emph{even for command lists that have been submitted}. In addition, instrument command lists are supposed to be executed \textbf{exactly once}, so do not instrument command lists that are recorded once and get re-submitted multiple times.
